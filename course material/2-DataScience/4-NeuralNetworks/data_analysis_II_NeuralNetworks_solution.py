...
#%% TODO 1 and 2: 
# Experiment with different network architectures and mini_batch_sizes
...
for mbs in mini_batch_sizes:
    for nu in no_units:
        print(f'Train NN with {nu} units using mini-batch size of {mbs}...')
        mynn = keras.Sequential([
            keras.layers.Dense(units=nu, activation='relu', input_dim=no_inputs),  # Hidden layer with c[1] units
            keras.layers.Dense(units=1, activation='sigmoid')  # output layer with one unit
        ])
    
        mynn.compile(optimizer='Adam', loss='binary_crossentropy',
                     metrics=['binary_accuracy'])
        mynn.fit(x=x_train, y=y_train, batch_size=mbs, epochs=5,
                 validation_data=(x_val, y_val))
    
        _, binary_accuracy = mynn.evaluate(x=x_test, y=y_test)
        results[(mbs, nu)] = binary_accuracy
