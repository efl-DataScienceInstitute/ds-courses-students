...
# %%
# Define a function that consists of model definition and training loop

def define_and_train_nn(train_x, train_y, units, mini_batch_size, no_epochs):

    nn = MLPClassifier(
        hidden_layer_sizes=(units,), activation='relu', batch_size=mini_batch_size,
    )

    for epoch in range(1, no_epochs + 1):
        train_loss = []  # list to collect train loss for each batch
        for b_idx in range(mini_batch_size, len(y_train), mini_batch_size):
            idx_start = b_idx - mini_batch_size
            idx_end = b_idx
            x_batch = train_x[idx_start:idx_end]
            y_batch = train_y[idx_start:idx_end]
            nn.partial_fit(x_batch, y_batch, classes=[0, 1])
            train_loss.append(nn.loss_)
...
for mbs in mini_batch_sizes:
    for nu in no_units:
        ...
        trained_nn = define_and_train_nn(x_train, y_train, no_epochs=5, units=nu, mini_batch_size=batch_size)
        test_results = trained_nn.score(x_test, y_test)
        print(f'Test accuracy when using {nu} units and {mbs} mini batch size: {np.round(test_results, 4)}')
